...
const TermList = ({ FlashcardList }: TermListProps) => {
  ...
  return (
    <div className="max-w-[1200px] p-10 mx-auto rounded-2xl bg-white dark:bg-black shadow-md dark:shadow-none space-y-10">
      <div className="font-bold text-2xl">
        Từ vựng trong bài ({FlashcardList.length})
      </div>
      <div className="grid grid-cols-2 gap-4">
        {FlashcardList.map((term, index) => (
          <div
            key={index}
            className="min-h-28 rounded-xl border-[1px] flex justify-between items-center p-4 shadow-md dark:shadow-none border-primary dark:bg-secondary "
          >
