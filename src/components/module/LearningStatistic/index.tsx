...
// // import { getStudentLearningStats } from "@/app/api/dashboard/dashboard.api";
...
// //       const [logData, statsData, activityData, learningStats] =
// //         await Promise.all([
// //           getLearningLog({
...
// //       // Sắp xếp yearlyStats theo month tăng dần
// //       const sortedYearlyStats = learningStats.yearlyStats.sort((a, b) =>
...
// //                 <YearlyStatsChart
// //                   data={yearlyStats}
// //                   description="Số lượng quiz, tài liệu và flashcard hoàn thành trong 12 tháng gần nhất"
...
// import { WeeklyStatsChart } from "../charts/WeeklyStatsChart"; // New import
// import { YearlyStatsChart } from "../charts/YearlyStatsChart";
...
//       setActivityLog(activityData);
//       setVisibleActivities(activityData.slice(0, ITEMS_PER_PAGE));
//       setWeeklyStats(learningStats.weeklyStats);
//       setYearlyStats(sortedYearlyStats);
//       setCurrentPage(0);
//       dataFetchedRef.current = true;
//     } catch (err: any) {
...
//                 <WeeklyStatsChart
//                   data={weeklyStats}
//                   description="Số lượng quiz, tài liệu và flashcard hoàn thành trong 7 ngày gần nhất"
...
import { YearlyStatsChart } from "../charts/YearlyStatsChart";
import { LineChartComponent } from "../charts/LineChartQuizStat";
...
export default function DailyTracker() {
  ...
  const fetchData = async (forceRefresh = false) => {
    ...
    try {
      ...
      const [logData, statsData, activityData, learningStats, quizData] =
        await Promise.all([
          getLearningLog({
            token: session.user.token as string,
            date: currentDate,
      ...
      const sortedQuizStats = quizData.sort((a, b) =>
      ...
      setActivityLog(activityData);
      setVisibleActivities(activityData.slice(0, ITEMS_PER_PAGE));
      setWeeklyStats(sortedWeeklyStats);
      setYearlyStats(sortedYearlyStats);
      setQuizStats(sortedQuizStats);
      setCurrentPage(0);
      dataFetchedRef.current = true;
    } catch (err: any) {
      setError(err.message || "Không thể tải dữ liệu");
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                />
                <LineChartComponent
                  chartConfig={quizChartConfig}
                  data={quizStats}
                  dataKey="averageScore"
                  description="Điểm trung bình của các bài quiz trong 7 ngày gần nhất"
                  strokeColor="hsl(var(--chart-1))"
                  title="Thống kê điểm quiz"
                  xAxisKey="date"
                />
              </div>
            </div>
