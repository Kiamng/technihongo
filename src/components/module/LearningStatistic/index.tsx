...
export default function DailyTracker() {
  ...
  const initializeDailyLog = async () => {
    ...
    try {
      ...
      // Kiểm tra xem bản ghi cho ngày hiện tại đã tồn tại chưa
      try {
        await getLearningLog({
          token: session.user.token as string,
          date: currentDate,
        ...
        console.log(
          `[${new Date().toISOString()}] Log exists for ${currentDate}, skipping trackLearningLog`,
        );
        await fetchData(true); // Lấy dữ liệu nếu bản ghi đã tồn tại
